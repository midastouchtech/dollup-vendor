...
const CreateServicePage = ({ vendor, socket }) => {
  ...
  const handleSubmit = () => {
    ...
    if (socket && !isEmpty(details)) {
      ...
      socket.emit("CREATE_VENDOR_SERVICE", {
        id: uuid(),
        vendor,
        active: true,
        date: new Date(),
      ...
      socket.on("RECEIVE_CREATE_SERVICE_SUCCESS", () => {
        notification.success({
          key: details.name,
          message: "Success!",
          description: "Your new Service has been added to your store!",
        });
                ...
                <figure className="ps-block--form-box">
                  ...
                  <div className="ps-block__content">
                    ...
                    <div className="form-group">
                      ...
                      <ImgUploadWithCropper
                        src={details?.galeryImage1}
                        width="150"
                        height="150"
                        onUploadComplete={(src) => 
                          setDetail("galeryImage1", src)
                        }
                        onUploadError={(err) => console.log(err)}
                      />
                      </div>
                    ...
                    <ImgUploadWithCropper
                        src={details?.galeryImage2}
                        width="150"
                        height="150"
                        onUploadComplete={(src) => 
                          setDetail("galeryImage2", src)
                        }
                        onUploadError={(err) => console.log(err)}
                      />
                    </div>
